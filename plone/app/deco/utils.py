...
from z3c.form.interfaces import IEditForm, IFieldWidget, DISPLAY_MODE, HIDDEN_MODE
...
def extractFieldInformation(schema, context, request, prefix):
    ...
    if context is not None:
        for field in _getDisallowedFields(
            context, mergedTaggedValueDict(schema, READ_PERMISSIONS_KEY), ''
        ):
            omitted[field] = 'true'
    read_only = _getDisallowedFields(
        context, mergedTaggedValueDict(schema, WRITE_PERMISSIONS_KEY), ''
    )
