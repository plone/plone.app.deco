...
on your Plone website. At their core, they rely on semantic HTML and
publishable URLs. Below, we will provide an overview of the core concepts
that make up Deco, Blocks and Tiles.
...
layout into the referenced site layout, and then incorporating tiles into the
page.
...
it could be a verbatim chunk of HTML (as is usually the case with Deco-managed
pages), the result of rendering a page template, or some other dynamically
generated content. If it has a ``layout`` link in the head, it will be
transformed and merged into the site layout.
...
The ``@@page-site-layout`` view will calculate the correct page layout to
...
This still respects and global section layouts, but will not utilise the
...
an ``id`` corresponding to the placeholder id (the ``target`` attribute).
If there is an element in the body of the *page layout* with an id
...
**Note:** Any content in the body of the page outside a panel that is
registered in the site layout is discarded when panel merging takes place.
...
can only appear once, any corresponding element in the site layout is
replaced. For other elements, the page layout's head contents are merged into
the site layout after the site layout's own head content.
...
When discussing tiles used in Deco, there are three types of tiles commonly
mentioned:

Text tiles
    Static HTML markup (WYSIWYG-edited text) placed into the page or site
    layout directly. Strictly speaking, text tiles are not tiles in that
    they do not involve any tile fetching or merging - instead they are
    stored as part of the page or site layout. To the user, however, a text
    tile can be moved around and managed like any other.

Field tiles
    Renders the value of a metadata field such as the title or description.
    May be edited in-place in the page, but the value is stored in the
    underlying field and can be indexed in the catalog, used for navigation
    and so on. In practice, a field tile is an instance of the special tile
    ``plone.app.standardtiles.fields`` with the field name passed as a
    parameter.

App tiles
    Any other type of dynamic tile. Examples may include a folder listing,
    a media player, a poll and pretty much anything else you can think of.
...
Note that ``plone_app_standardtiles_calendar`` unique id used in both records,
and the reference to the tile name (``plone.app.standardtiles.calendar``),
which should correspond to the name used in the ``<plone:tile />`` ZCML
directive.
...
If a tile has configuration parameters, these will often be embedded in a
...
replace the tile placeholder. Any contents in the ``<head />`` tag will be
merged into the final rendered page's head section. This allows tiles to
...
``<body />`` element. In this case, the placeholder id in the tile link can
be skipped.
